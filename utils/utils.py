...
def restore_checkpoint(opt,model,load_name=None,resume=False):
    ...
    if resume:
        ...
        if "beta" in checkpoint["graph"].keys():
            print("reload_beta:{}".format(checkpoint["graph"]["beta"]))
            model.graph.beta.data=checkpoint["graph"]["beta"]
...
def get_contain_tri(opt,var,pt2D,pts2D):
    num_tri=len(var.tri[0])
    for i in range(num_tri):
        vertexes=pts2D[var.tri[0][i]]
        x1,y1 = -pt2D[0]+vertexes[0][0],-pt2D[1]+vertexes[0][1]
        x2, y2 = -pt2D[0]+vertexes[1][0], -pt2D[1]+vertexes[1][1]
        x3, y3 = -pt2D[0]+vertexes[2][0], -pt2D[1]+vertexes[2][1]
        # simple_judge
        if ((x1>0)&(x2>0)&(x3>0)|(x1<0)&(x2<0)&(x3<0))|((y1>0)&(y2>0)&(y3>0)|(y1<0)&(y2<0)&(y3<0)):
            continue
        cross_dot1 = (x1*y2-y1*x2)>0
        cross_dot2 = (x2 * y3 - y2 * x3)>0
        cross_dot3 = (x3 * y1 - y3 * x1)>0
        if ((cross_dot1==True)&(cross_dot2==True)&(cross_dot3==True))|((cross_dot1==False)&(cross_dot2==False)&(cross_dot3==False)):
            return i
    return num_tri

def initial_depth_unc(opt,var,depth,unc,num_sce_seq,mode="simple_init"):
    num_tri=len(var.tri[0])
    trix_bool_mat=getattr(var,"tri{}_bool_mat".format(num_sce_seq))
    coordxy=var.coordxy
    if num_sce_seq==0:
        pose=np.concatenate((np.eye(3),np.zeros((3,1))),axis=-1)
    else:
        pose=var.pose.squeeze().detach().cpu().numpy()
    for i in range(num_tri):
        vertexes3D=var.pts3D[:,var.tri[0][i]]
        vertexes4D=np.concatenate((vertexes3D,np.ones((1,3))),axis=0)
        vertexes3D=np.matmul(pose,vertexes4D)
        if mode=="simple_init":
            depth_init=vertexes3D[2,:].mean()
            x_indexes,y_indexes=torch.split(coordxy[trix_bool_mat[i][None, :]],1,dim=-1)
            depth[y_indexes,x_indexes]=depth_init
            unc[y_indexes,x_indexes] = depth_init-(10*depth_init/opt.NeRF2V.sample_intvs)
        elif mode=="plan_intersect_init":
            # wait for finishing
            return
