...
class Model():
    ...
    def build_networks(self,opt):
        graph = importlib.import_module("models.{}".format(opt.model))
    ...
    def summarize_loss(self,opt,var,loss):
        ...
        for key in loss:
            ...
            if opt.loss_weight[key] is not None:
                ...
                if key=="entropy":
                    if opt.loss_weight[key].descent==True:
                        loss_all += torch.exp(-5e3*loss.render)*loss[key]
                    else:
                        loss_all += 10 ** float(opt.loss_weight[key].weight) * loss[key]
                else:
...
class Graph(torch.nn.Module):
    ...
    def entropy_loss(self,distribution):
        entropy_loss=torch.sum(-distribution*torch.log(distribution+10e-16),dim=-2).mean()
        return entropy_loss