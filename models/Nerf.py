...
class Model(base.Model):
    ...
    def visualize(self,opt,var,step=0,split="train",eps=1e-10):
        if split == "val":
            ...
            H, W = opt.H, opt.W
        poses_all=self.train_data.all['pose']
        pts_all=self.train_data.all['obj_pcl'][0]
        color_all = self.train_data.all['obj_color'][0]
        # util_vis.vis_cameras_pts(opt,self.vis,step,poses_all,pts_all,color_all)
        # pdb.set_trace()
        self.vis.image((var.image*255).view(H,W,3).permute(2,0,1),win="img")
        if opt.tb:
            img_gt=var.image.view(-1,H,W,3).permute(0,3,1,2)
            util_vis.tb_image(opt,self.tb,step,split,"image",img_gt)
    ...
    def evaluate_full(self,opt,eps=1e-10):
        ...
        for i,batch in enumerate(loader):
            ...
            if opt.vis_point_cloud==True:
                intrinsics.append(var.intr.unsqueeze(0))
                depths.append(invdepth_map)
                images.append(rgb_map)
                poses.append(var.pose)
        ...
        if opt.vis_point_cloud==True:
            cret_ptc(opt,images,depths,poses,intrinsics,opt.nerf.depth.min_d,opt.nerf.depth.max_d)
    ...
    def generate_videos_synthesis(self,opt,eps=1e-10):
        ...
        if opt.data.dataset=="blender":
            ...
            for i,pose in enumerate(pose_novel_tqdm):
                ...
                # print("the rendering time is {}".format(str(t1-t0)))
                # pdb.set_trace()
                #---------------------------------------------------------
...
class Graph(base.Graph):
    ...
    def compute_loss(self,opt,var,ret,mode=None):
        ...
        # if opt.loss_weight.entropy is not None:
        #     loss.entropy=self.entropy_loss(var.prob)
            # pdb.set_trace()
        return loss
    ...
    def render(self,opt,pose,intr=None,ray_idx=None,mode=None):
        ...
        ret = edict(rgb=rgb,depth=depth,opacity=opacity,prob=prob) # [B,HW,K]
        ...
        if opt.nerf.fine_sampling:
            ...
            ret.update(rgb_fine=rgb_fine,depth_fine=depth_fine,opacity_fine=opacity_fine,prob_fine=prob_fine) # [B,HW,K]
        return ret

    def render_by_slices(self,opt,pose,intr=None,mode=None):
        ret_all = edict(rgb=[],depth=[],opacity=[],prob=[])
        H, W = opt.data.val_img_size
        if opt.nerf.fine_sampling:
    ...
    def sample_depth(self,opt,batch_size,num_rays=None):
        depth_min, depth_max = opt.nerf.depth.min_d,opt.nerf.depth.max_d
...
class NeRF(torch.nn.Module):
    ...
    def __init__(self,opt):
        ...
        self.out_sidenet=NeRF_outside(opt)
        self.bound=opt.VolSDF.obj_bounding_radius/5

    def define_network(self,opt):
        if opt.nerf.hash_version==True:
            ...
            self.in_dim_color = self.encoder_dir.n_output_dims + 64
    ...
    def forward(self,opt,points_3D,ray_unit=None,mode=None): # [B,...,3]
        if opt.nerf.hash_version==True:
            output_color=torch.ones_like(points_3D).to(torch.float32).cuda()
            output_sigma = torch.ones(*points_3D.shape[:-1]).to(torch.float32).cuda()
            dist=torch.norm(points_3D,dim=-1,keepdim=True)
            shape_pts = points_3D.shape
            mask_inside=dist<self.bound
            pts_inside=points_3D[mask_inside.squeeze(-1)]
            ray_unit_inside=ray_unit[mask_inside.squeeze(-1)]
            pts_outside = points_3D[~mask_inside.squeeze(-1)]
            ray_unit_outside = ray_unit[~mask_inside.squeeze(-1)]
            # sigma
            self.bound=opt.VolSDF.obj_bounding_radius
            x = (pts_inside + self.bound) / (2 * self.bound)  # to [0, 1]
            x = self.encoder(x.view(-1,3))
            h = self.sigma_net(x)

            # sigma = F.relu(h[..., 0])
            # print(h[..., 0].max())
            sigma = activation_function.trunc_exp(h[..., 0])
            geo_feat = h[..., 1:]

            # color
            d = (ray_unit_inside + 1) / 2  # tcnn SH encoding requires inputs to be in [0, 1]
            d = self.encoder_dir(d.view(-1,3))
            ...
            # cal the background
            color_outside,sigma_outside=self.out_sidenet(opt,pts_outside,ray_unit_outside)
            output_color[mask_inside.squeeze(-1)]=color
            output_color[~mask_inside.squeeze(-1)] = color_outside.to(torch.float32)
            output_sigma[mask_inside.squeeze(-1)]=sigma
            output_sigma[~mask_inside.squeeze(-1)]=sigma_outside.to(torch.float32)
            # pdb.set_trace()
            return output_color,output_sigma
        else:
            if opt.arch.posenc:
    ...
    def composite(self,opt,ray,rgb_samples,density_samples,depth_samples):
        ...
        if opt.nerf.setbg_opaque:
            rgb = rgb+opt.data.bgcolor*(1-opacity)       # the light of background
        # pdb.set_trace()
...
class NeRF_outside(torch.nn.Module):
    ...
    def define_network(self,opt):
        if opt.nerf.out_hash_version==True:
            self.encoder = tcnn.Encoding(
                n_input_dims=4,
                encoding_config={
                    "otype": "HashGrid",
    ...
    def forward(self,opt,points_3D,ray_unit=None,mode=None): # [B,...,3]
        if opt.nerf.out_hash_version==True:
            dist=torch.norm(points_3D,dim=-1,keepdim=True)
            pts=torch.cat([points_3D/dist,opt.VolSDF.obj_bounding_radius/5/dist],dim=-1)
            shape_pts=pts.shape
            # self.bound=opt.VolSDF.obj_bounding_radius
            x = (pts + 1) / 2  # to [0, 1]
            x = self.encoder(x.view(-1,4))
            h = self.sigma_net(x)
            ...
            # sigmoid activation for rgb
            color = torch.sigmoid(h)
            return color.view([*shape_pts[:-1],3]),sigma.view(*shape_pts[:-1])
        else:
            if opt.arch.posenc:
                points_enc = self.positional_encoding(opt,points_3D,L=opt.arch.posenc.L_3D)
                points_enc = torch.cat([points_3D,points_enc],dim=-1) # [B,...,6L+3]
